...
const BusCard = (props) => {
  ...
  const viewSeats = (getSeats) => {
  console.log(getSeats,"seats") 
  getSeats.seatCount.length > 0 &&  setBerth(true)
  setId(getSeats._id)
  setBerthCount(getSeats)
  } 
  return (
    <div className="mb-6">
      {datas.data.slice(0, moreDeals).map((cardDatas, index) => (
        <div className="border relative w-full grid grid-rows-1 my-5 transition duration-300  hover:border-[#ec476e]  py-2 px-2 ">
          <div className="flex items-center py-1 ">
            <div className="w-1/4">
              <p className="text-[14px] font-bold">{cardDatas.busName}</p>
              <div className="flex items-center gap-2">
                <p className="text-[14px] ">{cardDatas.busType}</p>
                <p className="text-[14px] text-yellow-400 font-semibold">{cardDatas.layout}</p>
              </div>
              <div className="xl:w-[22%] 2xl:w-[17%] my-1 ">
                <span className="text-[13px] flex items-center bg-yellow-600  text-white font-semibold rounded-lg">
          ...
          <div className="relative">
            ...
            {cardDatas.Amenities.map((amenities) => (
              <div className="w-[80%] grid grid-cols-3 items-center gap-1">
                ...
                {amenities.blanketsStatus ? <p className="flex items-center gap-1"><img src={Blankets} className="w-7 h-6" /> <span className="text-[10px]  hover:underline">{amenities.blanketsStatus}</span></p> : ""}
              </div>
            ))}
        ...
        { berth !== true ? <div>
            ...
            cardDatas._id == id &&
            <div>
              <button className="absolute  duration-100 top-56 right-1  border rounded-full text-[#ff1a4f] text-sm p-[0.20rem] hover:bg-[#ff1a4f] hover:text-white border-[#ff1a4f]" onClick={()=>setBerth(false)}><Close/></button> </div>}
